"""Plot favored and unfavored pion Collins TMD bands at fixed z and Q2."""
...
from Collins import fixed_z_weighted_collins_rows, linspace, load_collins_fit
...
def plot_rows(
    ...
    for flavor in flavors:
        ...
        color = colors[flavor]
        ax.fill_between(pt_grid, lower, upper, color=color, alpha=0.32, label=flavor)
    ...
    ax.set_title("JAM3D pion Collins TMD at fixed z", pad=10)
    ax.set_xlabel(r"$p_T\ \mathrm{(GeV)}$")
    ax.set_ylabel(r"$\frac{z^2 p_T^2}{2M_\pi^2}H_1^\perp(z,z p_T)$")
...
def main() -> None:
    ...
    TAG = "JAM3D_2022"
    CSV_OUTPUT = output_path("fav_unf_fixed_z_collins.csv")
    PLOT_OUTPUT = output_path("fav_unf_fixed_z_collins.png")

    tmd, replicas, jam3dlib_path, jam3d_dev_lib_path = load_collins_fit(
        tag=TAG,
        replica_selection=REPLICA_SELECTION,
    )
    rows = fixed_z_weighted_collins_rows(
        tmd=tmd,
        z=Z,
        pt_grid=PT_GRID,
        q2=Q2,
    ...
    output_rows = [
        {
            ...
            "nrep": row["nrep"],
            "weighted_collins_mean": row["mean"],
            "weighted_collins_std": row["std"],
        }
        for row in rows
    ]
    write_rows(
        ...
        fieldnames=[
            ...
            "flavor_index",
            "nrep",
            "weighted_collins_mean",
            "weighted_collins_std",
        ],
    )
