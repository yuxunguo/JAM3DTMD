...
*...INTERPOLATION :
      XT(1) = DLOG(X)
      XT(2) = DLOG(Q2)
      U = FINT(NARG,XT,NA,ARRF,XUF) * (1.D0-X)**3 * X
      D = FINT(NARG,XT,NA,ARRF,XDF) * (1.D0-X)**4 * X
      UB = FINT(NARG,XT,NA,ARRF,XUBF) * (1.D0-X)**8 * X**0.5
      DB = FINT(NARG,XT,NA,ARRF,XDBF)  * (1.D0-X)**8 * X**0.5
      ST = FINT(NARG,XT,NA,ARRF,XSF)  * (1.D0-X)**8 * X**0.5
 60   RETURN
      END