...
from scipy.integrate import quad, dblquad, fixed_quad, simpson, romb
...
def get_interpolation(xF, pT, rs, tar, had): #possibly add function argument
    reps = 300
    grid_x, grid_z = np.mgrid[0.1:1:20j, 0.1:1:20j]
    rng = np.random.default_rng()
    grid = rng.random((reps, 2))
    #print(grid)
    xpoints = np.array(0.1 + (grid *(1.0 - 0.1))).T[0]
    zpoints = np.array(0.1 + (grid *(1.0 - 0.1))).T[1]
    values = []
    for i in range(reps):
        #for j in zpoints:
        values = np.append(values,[get_dsigST(0.1 + (grid[i,0] * (1.0 - 0.1)), 0.1 + (grid[i,1] * (1.0 - 0.1)), xF, pT, rs, tar, had)])
        #print(get_dsigST(grid[i,0], grid[i,1], xF, pT, rs, tar, had))
        #print(0.2 + (grid[i,0] * (1.0 - 0.2)), 0.2 + (grid[i,1] * (1.0 - 0.2)))
        #print('RESET')
        
    #print(values)
    return griddata(grid, values, (grid_x, grid_z), method='cubic', fill_value =0)
...
def get_sigST(xF, pT, rs, tar, had, mode='gauss', nx=10, nz=10):
    ...
    if had=='jet':
        ...
        if mode == 'gauss':
            #looking to integrate griddata[i] by x
            #then integrate across all of these values by z
            integrands = get_interpolation(xF, pT, rs, tar, had)
            #grid_x, grid_z = np.mgrid[0:1:50j, 0:1:50j]
            grid_x = np.linspace(0.1, 1.0, 20)
            grid_z = np.linspace(0.1, 1.0, 20)
            #print(grid_z[10])
            #print(get_interpolation(xF, pT, rs, tar, had)[10,10])
            #print(get_dsigST(0.574, 0.574, xF, pT, rs, tar, had))
            zintegrand = []
            #for i in range(len(integrands)):
                #zintegrand = np.append(zintegrand, simpson(get_interpolation(xF, pT, rs, tar, had)[i], grid_x))
            xzintegrand = []
            zintegrand = []
            xintegrand = []
            z_line = []
            ...
            for k in range(len(grid_z)):
                if grid_z[k] < zmin:
                    z_line = np.delete(grid_z, k, 0)
                    xzintegrand = np.delete(integrands, k, 0)
                xintegrand = []
                for d in range(len(z_line)):
                    for j in range(len(grid_x)):
                    #print(j)
                    #print(grid_x[j])
                        if grid_x[j] < xmin(grid_z[d]):
                            x_line = np.delete(grid_x, j, 0)
                            xintegrand = np.delete(xzintegrand[d], j, 0)
                    zintegrand = np.append(zintegrand, simpson(get_interpolation(xF, pT, rs, tar, had)[k], grid_x))    
            sig = simpson(zintegrand, grid_z)
            #sig = 10
#             start = time.time()
...
if __name__ == '__main__':
  ...
  def test():
    ...
    #end1 =time.time()
    num = get_sigST(xF, pT, rs, tar, had, mode='gauss', nx=10, nz=10)
    end2 = time.time()
    #print('den=', end1-start)
    #print('num=', end2-end1)
    print('total = ', end2 - start)
    AN = num / den
    #yesyes = get_dsigST(0.5, 0.5, xF, pT, rs, tar, had)
    #nono = get_interpolation(xF, pT, rs, tar, had)
    #print(yesyes)
    #print(nono)
    #print(num)
    print(AN)
...
# print 'time=',(end-start)
  ...
  for k in range(50):
      ...
      #print('time=', (end - start))
  #ending = time.time()
  #print('total time=', (ending-beginning))
